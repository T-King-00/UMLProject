    ...
    for token in doc:
        if token.pos_ == "NOUN":
            ...
            if token.dep_ in deps_attr:
                classes_attr = add_to_attributes ( classes_attr, text, token.lemma_ )
            # class
            elif token.dep_ in deps_class:
                classes_attr = add_to_classes ( classes_attr, text, token.lemma_ )
    return classes_attr
    ...
    for sent in doc.sents:
        ...
        for i in range ( len ( children ) ):
            for j in range ( i + 1, len ( children ) ):
                if children [ i ].lemma_ in classes and children [ j ].lemma_ in classes and children [ i ].lemma_ != \
                        children [ j ].lemma_:
